...
(async function boot(){
  try{
    ...
    if(session && session.user){
      ...
      // Show the app even if one of the cloud tables is missing or a query fails.
      // This prevents the page from getting stuck on only the header after login.
      document.getElementById('auth-view').classList.add('hidden');
      document.getElementById('app-view').classList.remove('hidden');
      document.getElementById('logout-btn').classList.remove('hidden');
      const pill = document.getElementById('sync-status');
      pill.textContent = currentUser.email ? currentUser.email.split('@')[0] : 'Logged in';
      pill.classList.add('ok');

      try{
        await loadRemote();
      }catch(loadErr){
        console.error('Cloud load failed; falling back to local data:', loadErr);
        loadLocal();
      }
      initUI();
      return;
    ...
    document.getElementById('sync-status').textContent = 'Logged out';
  }catch(e){
